...
```{r load_data}
births = read.csv("code/data/US_births(2018).csv")
```
...
```{r}
write.csv(births_clean, "code/data/births_clean.csv", row.names=F)
```
...
Null hypothesis: There is no relationship between the X variables we examined
and the birth weight of the infant.
...
```{r}
which_vars_x<- new_col_names[!new_col_names %in% c("birth_weight_g")]
...
```{r}
lm(formula=form, data = births_no_na)
```
```{r}
summary(lm(formula=form, data = births_no_na))
```
